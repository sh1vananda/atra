...
export default function AdminDashboardPage() {
  ...
  const StatCard = ({ title, value, icon: Icon, onClick }: { title: string; value: string | number; icon: React.ElementType; onClick?: () => void }) => (
    <Card 
        className={`bg-card shadow hover:shadow-lg transition-shadow ${onClick ? 'cursor-pointer hover:border-primary' : ''}`}
        onClick={onClick}
    ...
    > 
      ...
      <CardContent> <div className="text-2xl font-bold">{value}</div> <p className="text-xs text-muted-foreground"> in {managedBusiness?.name} </p> </CardContent> 
    </Card>
  );
  
  const navigateToUserManagement = () => {
    setCurrentView("userManagement");
    // Optional: scroll to the UserTable or a specific part of the page
